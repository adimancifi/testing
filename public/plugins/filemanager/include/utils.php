...
function create_folder($path = null, $path_thumbs = null,$ftp = null,$config = null)
{
	if($ftp){
		...
		{
			$mkdirr = mkdir($path, $permission, true);
			if ($mkdirr == true && !file_exists($path.'/index.html')) {
				...
				if (is_resource($handle)) {
					...
					dei("$handle : bukan resoure !");
				}
			}
		} // or even 01777 so you get the sticky bit set
		if ($path_thumbs)
		{
			$mkdirr = mkdir($path_thumbs, $permission, true) or die("$path_thumbs cannot be found");

			if ($mkdirr == true && !file_exists($path_thumbs.'/index.html')) {

				$content = '<html><head><title>403 Forbidden</title></head><body><p>Directory access is forbidden.</p></body></html>';
				$handle = fopen($path_thumbs.'/index.html', 'w');
				if (is_resource($handle)) {
					fwrite($handle, $content);
					fclose($handle);
				} else {
...
* @return  bool
*/
function is_really_writable_2($dir)
{
	$dir = rtrim($dir, '/');
	// linux, safe off
...
* @return  bool
*/
function is_php_2($version = '5.0.0')
{
	static $phpVer;
